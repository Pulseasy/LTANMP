...
if [ -s mariadb-$mariadb_version.tar.gz ]; then
  ...
  wget -c http://mirrors.opencas.cn/mariadb/mariadb-$mariadb_version/source/mariadb-$mariadb_version.tar.gz
  dl_status=`echo $?`
  if [ $dl_status = "0" ]; then
...
if [ -s /usr/local/bin/cmake -o -s /usr/bin/cmake ]; then
  ...
  wget -c http://www.cmake.org/files/v3.2/cmake-3.2.2.tar.gz
...
cmake . -DCMAKE_INSTALL_PREFIX=/usr/local/mysql -DMYSQL_DATADIR=/usr/local/mysql/data -DWITH_ARIA_STORAGE_ENGINE=1 -DWITH_XTRADB_STORAGE_ENGINE=1 -DWITH_ARCHIVE_STORAGE_ENGINE=1 -DWITH_INNOBASE_STORAGE_ENGINE=1 -DWITH_PARTITION_STORAGE_ENGINE=1 -DWITH_FEDERATED_STORAGE_ENGINE=1 -DWITH_BLACKHOLE_STORAGE_ENGINE=1 -DWITH_MYISAM_STORAGE_ENGINE=1 -DWITH_READLINE=1 -DENABLED_LOCAL_INFILE=1 -DDEFAULT_CHARSET=utf8 -DDEFAULT_COLLATION=utf8_general_ci -DWITH_EMBEDDED_SERVER=1 -DWITH_SSL=system 
make
